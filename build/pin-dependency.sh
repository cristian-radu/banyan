...
# Usage:
#   build/pin-dependency.sh $MODULE $SHA-OR-TAG
#
# Example:
#   build/pin-dependency.sh github.com/docker/docker 501cb131a7b7
...
if [[ -z "${dep}" || -z "${sha}" ]]; then
  echo "Usage:"
  echo "  build/pin-dependency.sh \$MODULE \$SHA-OR-TAG"
  echo ""
  echo "Example:"
  echo "  build/pin-dependency.sh github.com/docker/docker 501cb131a7b7"
  echo ""
  exit 1
fi

declare -r projectRoot=$(dirname $(dirname $(readlink -fm $0)))
_tmp="${projectRoot}/_tmp"
cleanup() {
  rm -rf "${_tmp}"
}
